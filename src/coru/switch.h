...
#define STACK_BASE(stack_start, length)		\
	((unsigned char*)(stack_start) + (length))
#define FRAME_LENGTH(stack_base, frame)			\
	((unsigned char*)(stack_base) - (unsigned char*)(frame))
#define STACK_CHAR(stack_base, index)			\
	(((unsigned char*)(stack_base))[-(index)-1])
#else  /* STACK_GROWS_UPWARD */
